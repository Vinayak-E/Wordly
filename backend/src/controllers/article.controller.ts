  ...
  try {
    ...
    if (!ALLOWED_IMAGE_TYPES.includes(file.mimetype)) {
      res
        ...
        .json({
          message:
            "Invalid file type. Only image files (JPEG, PNG, GIF, WebP, BMP) are allowed.",
        });
      return;
    }
      ...
      category: {
        $in:
          userPreferences.length > 0
            ? userPreferences
            : await Article.distinct("category"),
      },
    };
...
export const dislikeArticle = async (
  ...
  try {
    ...
    if (article.dislikes.includes(user._id)) {
      res.status(400).json({ message: "Article already disliked" });
      return;
    }

    article.dislikes.push(user._id);
